...
AddEventHandler('qb-vehicleshop:client:orderVehicle', function(vehicle, data)
    ...
    local data = {
        ['content'] = 'Có khách đặt xe kìa shop! @Cardealer ',
        ['embeds'] = {
            {
                ['title'] = 'Loại Xe Khách Yêu Cầu',
                ['description'] = 'Kiểm tra danh sách bên dưới!',
    ...
    PerformHttpRequest(webhookUrl, function(statusCode, response, headers)
        TriggerClientEvent('QBCore:Notify', src, 'Order của bạn đã được chuyển đến cardealer!', "success")
...
RegisterServerEvent('qb-vehicleshop:server:buy-stock', function(vehicle)
    ...
    if account >= discount  then
        ...
        exports["qb-banking"]:RemoveMoney(tostring(Player.PlayerData.job.name), discount)
...
RegisterNetEvent('qb-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    ...
    if cash > tonumber(vehiclePrice) then
        ...
        exports['qb-banking']:AddMoney(player.PlayerData.job.name, vehiclePrice)
